...
int bpInitialized = 0, srInitialized = 0;    
unsigned long ofs;
...
- jack_get_buffer_size( jackHostApi->jack_client )   
...
stream->t0 = jack_frame_time( jackHostApi->jack_client );    
...
int chn;
int framesProcessed;
const double sr = jack_get_sample_rate( stream->jack_client );     
...
else if( stream->doStop || stream->doAbort )     
{
if( stream->callbackResult == paContinue )      
{
