...
if (window->SkipItems || !window->ClipRect.Overlaps(window->DC.LastItemRect))  
return false;
IM_ASSERT(window->DC.LastItemRect.Min.x == preview_data->PreviewRect.Min.x && window->DC.LastItemRect.Min.y == preview_data->PreviewRect.Min.y);  
...
IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DisplayMask));     
IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DataTypeMask));    
IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__PickerMask));      
IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__InputMask));       
...
want_close = true;
if (!menu_is_open && hovered && pressed)  
want_open = true;
